...
"""
            ...
            <div class="js-analysis">
                <div class="js-stats">
                    ...
                    <div class="stat-item">
                        ...
                        <span class="stat-value">{len(js_analysis.frameworks)}</span>
                    </div>
                    <div class="stat-item">
                ...
                {f'<div class="frameworks-detected"><h4>Frameworks Detected:</h4><ul>{"".join([f"<li>{fw.name}</li>" for fw in js_analysis.frameworks])}</ul></div>' if js_analysis.frameworks else ''}
            </div>
        </div>
        """
    ...
    def _generate_markdown_report(self, data: ReportData) -> str:
        ...
        markdown_content = f"""# Web Accessibility Analysis Report

**URL:** {data.url}  
**Analysis Type:** {data.analysis_type}  
**Generated:** {data.timestamp}  
**Report ID:** {data.analysis_id}
...
"""
        ...
        if data.score:
            markdown_content += f"""### Overall Scores
- **Scraper Accessibility:** {data.score.scraper_friendliness.total_score:.1f}/100 ({data.score.scraper_friendliness.grade})
- **LLM Accessibility:** {data.score.llm_accessibility.total_score:.1f}/100 ({data.score.llm_accessibility.grade})

"""
        
        # Add enhanced LLM score if available
        if data.enhanced_llm_report:
            markdown_content += f"""### Enhanced LLM Analysis
- **Overall Score:** {data.enhanced_llm_report.overall_score:.1f}/100 ({data.enhanced_llm_report.grade})
- **Content Quality:** {data.enhanced_llm_report.content_quality_score}/100
- **Semantic Structure:** {data.enhanced_llm_report.semantic_structure_score}/100
- **Structured Data:** {data.enhanced_llm_report.structured_data_score}/100
...
"""
        ...
        if data.static_result and data.static_result.content_analysis:
            ...
            markdown_content += f"""## Content Analysis
...
"""
        ...
        if data.static_result and data.static_result.javascript_analysis:
            ...
            markdown_content += f"""## JavaScript Analysis
- **Total Scripts:** {js.total_scripts}
- **Inline Scripts:** {js.inline_scripts}
- **External Scripts:** {js.external_scripts}
- **Frameworks Detected:** {len(js.frameworks)}
- **SPA Pattern:** {'Yes' if js.is_spa else 'No'}
- **AJAX Usage:** {'Yes' if js.has_ajax else 'No'}

"""
        
        # Add SSR detection
        if data.ssr_detection:
            markdown_content += f"""## Rendering Analysis
- **Server-Side Rendering:** {'Yes' if data.ssr_detection.is_ssr else 'No'}
- **Confidence:** {data.ssr_detection.confidence:.1%}
- **Rendering Type:** {data.ssr_detection.rendering_type}
...
"""
        ...
        if data.evidence_report:
            markdown_content += f"""## Evidence Report
- **Analysis ID:** {data.evidence_report.analysis_id}
- **Crawlers Tested:** {data.evidence_report.summary.get('total_crawlers', 0)}
- **Total Issues:** {data.evidence_report.summary.get('total_issues', 0)}

"""
        
        markdown_content += f"""---
*Report generated by Web Scraper & LLM Analyzer on {data.timestamp}*
"""
    ...
    def _extract_key_findings(self, data: ReportData) -> List[str]:
        ...
        if data.static_result and data.static_result.javascript_analysis:
            js = data.static_result.javascript_analysis
            findings.append(f"JavaScript: {js.total_scripts} scripts, {len(js.frameworks)} frameworks detected")
